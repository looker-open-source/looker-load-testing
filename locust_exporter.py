...
if __name__ == '__main__':
    ...
    listener_port = os.environ["LISTEN_PORT"]
    locust_host = os.environ["LOCUST_HOST"]
    try:
        start_http_server(int(listener_port))
        REGISTRY.register(LocustCollector(f"{locust_host}:8089"))
        print(f"Connecting to locust on: {locust_host}:8089")
        while True:
            time.sleep(1)
